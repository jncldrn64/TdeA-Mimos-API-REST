...
de TL-FCCU decía "Bash, documentado en inglés" y ese repo ya sumó un agente Java. Peor, las
dos secciones insinúan una dependencia entre repos que no existe: el estándar que estos
...
CHANGELOG no se reescribe: las menciones a TdeA-Mimos-Website, TL-FCCU y MIDI-Scale-Trainer
en v1.0 y v1.1 son procedencia y quedan.
...
**Contexto:** los dos repos Mimos (este y TdeA-Mimos-Website) borraron los nombres de
otros repos de sus docs para que cada documento se sostenga solo ante un lector sin
...
la vez, este y TdeA-Mimos-Website, como excepción explícita a "Scope de escritura", que
...
no alcanza: la cabecera de la v1.0 del CHANGELOG nombra a TdeA-Mimos-Website, TL-FCCU y
MIDI-Scale-Trainer, y esa historia no se reescribe, así que la referencia sobrevive; mejor
que resuelva contra un anclaje. No se revierte el borrado previo de los nombres en los docs
(commit `0a56eb4`): la política rige hacia adelante y las menciones que quedan en el
CHANGELOG ahora resuelven contra "Repos hermanos".
